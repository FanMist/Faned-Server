...
static void text_color(char c, char *stack, int size, int &sp, bvec color, int a) 
{
    ...
    else
    {
        ...
        switch(c)
        {
            ...
            case 'A': color = bvec(192,  96, 255); break;   // newcolor: violet;
            case 'B': color = bvec(224,  64, 224); break;   // newcolor: purple;
            case 'C': color = bvec(164,  72,  56); break;   // newcolor: brown;
            case 'D': color = bvec( 86, 164,  56); break;   // newcolor: dark green;
            case 'E': color = bvec( 56,  64, 172); break;   // newcolor: dark blue;
            case 'F': color = bvec(172, 172,   0); break;   // newcolor: dark yellow;
            case 'G': color = bvec(172,  56,  56); break;   // newcolor: dark red;
            case 'H': color = bvec(172,  72, 172); break;   // newcolor: dark magenta;
            case 'I': color = bvec(172,  56,   0); break;   // newcolor: dark orange;
            case 'J': color = bvec(48,  172, 172); break;   // newcolor: dark cyan;
            case 'K': color = bvec(102, 102, 102); break;   // newcolor: dark grey;
            case 'L': color = bvec(0,     0,   0); break;   // newcolor: black;
            case 'R': color = bvec((rand() % 255), (rand() % 255), (rand() % 255)); break; // newcolor: blinking rainbow;
            case 'X': { int m = lastmillis / 250; color = bvec( 64, 255, 128); a = m % 2 ? 96 : 255; break; } // newcolor: blinking green;
            case 'Y': { int m = lastmillis / 250; color = bvec( 96, 160, 255); a = m % 2 ? 96 : 255; break; } // newcolor: blinking blue;
            case 'Z': { int m = lastmillis / 250; color = bvec(255,  64,  64); a = m % 2 ? 96 : 255; break; } // newcolor: blinking red;
            //end faned
