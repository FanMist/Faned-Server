...
#define GUI_TITLE_COLOR  0x50CFE5    //faned
#define GUI_BUTTON_COLOR 0xFFFFFF    //faned
#define GUI_TEXT_COLOR   0xFFFFFF    //faned
...
void guislider(char *var, int *min, int *max, char *onchange, int *color)
{
	if(!cgui) return;
    int oldval = getval(var), val = oldval, vmin = *max > INT_MIN ? *min : getvarmin(var), vmax = *max > INT_MIN ? *max : getvarmax(var);
    cgui->slider(val, vmin, vmax, (*color ? *color : GUI_TITLE_COLOR));
