...
void setmapfilenames(const char *fname, const char *cname = 0)
{
    ...
    string backupstr;
    time_t t = time(NULL);
    size_t len = strftime(backupstr, sizeof(backupstr), "%Y-%m-%d_%H.%M.%S", localtime(&t));
    backupstr[min(len, sizeof(backupstr)-1)] = '\0';

    formatstring(ogzname)("packages/%s.ogz", mapname);
    if(savebak == 1) formatstring(bakname)("packages/%s.BAK", mapname);
    else if(savebak == 2) formatstring(bakname)("packages/%s_%d.BAK", mapname, totalmillis);
    else if(savebak == 3) formatstring(bakname)("packages/%s_%s.BAK", mapname, backupstr);
    formatstring(cfgname)("packages/%s/%s.cfg", pakname, mcfgname);
    formatstring(picname)("packages/%s.jpg", mapname);
    if (!fileexists(picname, "e")) formatstring(picname)("packages/%s.png", mapname);
...
//start faned
FANED_VARP(savemapshot, 0, 0, 1);

bool save_world(const char *mname, bool nolms)
{
    if(!*mname) mname = game::getclientmap();
    defformatstring(noname)("untitled_%d", totalmillis);
    setmapfilenames(*mname ? mname : noname);
    if(savebak) backup(ogzname, bakname);
    stream *f = opengzfile(ogzname, "wb");
    if(!f) { conoutf(CON_WARN, "\f8faned\f7|save_world: could not write map to %s", ogzname); return false; }
    if(savemapshot)
    {
        extern void screenshot(char*);
        extern char* screenshotdir;
        defformatstring(oldscreenshotdir)("%s", screenshotdir);
        defformatstring(screenshotname)("%s.png", *mname ? mname : noname);      
        formatstring(screenshotdir)("%s", "packages/base/");
        screenshot(screenshotname);
        formatstring(screenshotdir)("%s", oldscreenshotdir);
        conoutf(CON_INFO, "\f8faned\f7|save_world: mapshot \f2%s\f7 saved", screenshotname);
    ...
    delete f;
    conoutf("\f8faned\f7|save_world: wrote map file %s", ogzname);    //faned
    return true;
}
