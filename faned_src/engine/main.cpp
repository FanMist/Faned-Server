...
//start faned
FANED_SVARP(backgroundimage, "faned/data/background.png");
FANED_SVARP(backgroundlogo, "faned/data/logo.png");
...
void renderprogress(float bar, const char *text, GLuint tex, bool background)   // also used during loading
{
    ...
    settexture("faned/data/loading_frame.png", 3);    //faned
    glBegin(GL_TRIANGLE_STRIP);
    ...
    if(bar > 0)
    {
        settexture("faned/data/loading_bar.png", 3);    //faned
        glBegin(GL_QUADS);
...
int main(int argc, char **argv)
{
    ...
    logoutf("init: scripts");                //faned
    execfile(game::fanedmain(), false);      //faned
    execfile(game::fanedconfig(), false);    //faned
    execfile("faned/scripts/sounds.cfg");    //faned
