...
void writecfg(const char *name)
{
    ...
    stream *fanedf = openutf8file(path("sauer_faned_config.cfg", true), "w");
    if(!f) return;
    f->printf("// automatically written on exit, DO NOT MODIFY\n// delete this file to have %s overwrite these settings\n// modify settings in game, or put settings in %s to override anything\n\n", game::defaultconfig(), game::autoexec());
    if(!fanedf) return;
    else fanedf->printf("// FANATIC EDITION: sauer_faned_config.cfg\n// Automatically written on exit; DO NOT MODIFY.\n\n");
    ...
    {
        ident &id = *ids[i];
        stream* t = id.flags&IDF_FANED?fanedf:f;
