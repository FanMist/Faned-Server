...
FANED_VARP(consmoothfade, 0, 1, 1);    //faned
FANED_VARP(contimestamp, 0, 1, 1);    //faned
...
ICOMMAND(getconline, "", (), result(line ? line : ""));
//end faned
...
VARP(fullconsize, 0, 70, 100);    //faned
...
int drawconlines(int conskip, int confade, int conwidth, int conheight, int conoff, int filter, int y = 0, int dir = 1)
{
    ...
    loopi(numl) //determine visible height
    {
        ...
        //start faned
        string line;
        if(contimestamp && fullconsole && conlines[idx].time && conlines[idx].time[0]) formatstring(line)("\fs\f4[%s]\fr %s", conlines[idx].time, conlines[idx].line);
    ...
    loopi(numl)
    {
        ...
        if(consmoothfade) {
                ...
                alpha = 255.0 - 255.0*((float(totalmillis-conlines[idx].outtime)/1000.0)-float(confade)/10.0*9.0)/(float(confade)/10.0);
