...
struct gui : g3d_gui
{
        ...
        if(visible())
        {
            ...
            if(overlaid)
            {
                if(hit)
                {
                    ...
                    glColor3f(0.5f, 0.5f, 0.5f);    //faned
                    rect_(xi, yi, xs, ys);
                    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
                    glEnable(GL_TEXTURE_2D);
                    defaultshader->set();
    ...
    {	
        ...
        if(visible() && !layoutpass)
        {
            ...
            glDisable(GL_TEXTURE_2D);
            glDisable(GL_BLEND);
            if(editing) glColor3f(1, 1, 1);    //faned
    ...
    void icon_(Texture *t, bool overlaid, int x, int y, int size, bool hit)
    {
        ...
        x += int((size-xs)/2);
        y += int((size-ys)/2);
        const vec color = vec(1, 1, 1);    //faned
    ...
    int button_(const char *text, int color, const char *icon, bool clickable, bool center)
    {
        ...
        if(visible())
        {
            ...
            if(hit && clickable) color = 0x555555;    //faned
            int x = curx;	
