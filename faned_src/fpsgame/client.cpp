...
namespace game
{
    ...
    FVARP(minimapalpha, 0, 0.5, 1);    //faned
    ...
    void editmute(int cn)
    {
        ...
        conoutf(CON_INFO, "\f8faned\f7|editmute: ignoring %s", d->name);
    ...
    void uneditmute(int cn)
    {
        ...
        fpsent *d = getclient(cn);
        if(d) conoutf(CON_INFO, "\f8faned\f7|editmute: stopped ignoring %s", d->name);
    ...
    void printvar(fpsent *d, ident *id)
    {
        if(id) switch(id->type)
        {
            ...
            case ID_SVAR:
                ...
                if(detect) conoutf(CON_INFO, "\f8faned\f7|ID_SVAR: %s", escapestring(*id->storage.s));
                break;
        }
    }
    ...
    void toserver(char *text) {
        if(player1->state==CS_SPECTATOR) {
            conoutf(CON_CHAT, "%s%s \fs\f5(%d%s)\fr\f4: %s",
            ...
            conoutf(CON_CHAT, "%s%s \fs\f5(%d%s)\fr\f7: %s",
                m_check(game::gamemode, M_TEAM) ? "\fY>\f1" : "\fX>\f0",
                colorname(player1),
                player1->clientnum,
                    isadmin(player1->clientnum) ?  ", Admin" :
            ...
            particle_textcopy(player1->abovehead(), text, PART_TEXT, 2000, 0x32FF64, 4.0f, -8);
        }
        addmsg(N_TEXT, "rcs", player1, text);
        if(identexists("on_chat")) execute("on_chat"); }
    COMMANDN(say, toserver, "C");

    void sayteam(char *text) {
        if(player1->state==CS_SPECTATOR) {
            conoutf(CON_CHAT, "\fY>\f1%s \fs\f5(%d%s)\fr\f4: %s",
                colorname(player1),
                player1->clientnum,
                    isadmin(player1->clientnum) ?  ", Admin" :
                    (
            ...
            conoutf(CON_CHAT, "\fY>\f1%s \fs\f5(%d%s)\fr\f1: %s",
                colorname(player1),
                player1->clientnum,
                    isadmin(player1->clientnum) ?  ", Admin" :
                    (
        ...
        if(identexists("on_chat_team")) execute("on_chat_team"); }
    COMMAND(sayteam, "C");
    //end faned
    ...
    //start faned
    FANED_VARP(receiveai, 0, 0, 1);
    FANED_VARP(receivenewmap, 0, 0, 1);
    FANED_VARP(receiveremip, 0, 0, 1);
    FANED_VARP(receivesendmap, 0, 0, 1);
    //end faned
    ...
    void parsemessages(int cn, fpsent *d, ucharbuf &p)
    {
        ...
        while(p.remaining()) switch(type = getint(p))
        {
            ...
            case N_TEXT:
            {
                ...
                //start faned
                if(d->state!=CS_SPECTATOR) particle_textcopy(d->abovehead(), text, PART_TEXT, 2000, 0x32FF64, 4.0f, -8);
            ...
            case N_SAYTEAM:
            {
                ...
                if(t->state==CS_SPECTATOR) {
                    conoutf(CON_CHAT, "\f1%s \fs\f5(%d%s)\fr\f4: %s",
                        colorname(t),
                        t->clientnum,
                    ...
                    conoutf(CON_CHAT, "\f1%s \fs\f5(%d%s)\fr\f1: %s",
                        colorname(t),
                        t->clientnum,
                        isadmin(t->clientnum) ?  ", Admin" :
                        (
            ...
            case N_SHOTFX:
            {
                ...
                //start faned
                if(detect && !m_edit && from.dist(to) <= 1)
                {
                    conoutf(CON_WARN, "\f8faned\f7|detect: Client \f2%s \f5(%d)\f7, Type: \f2from.dist(to) <= 1\f7, AKA: \f2Telekill", s->name, s->clientnum);
                }
                if(detect && !m_edit && from.dist(to) > guns[gun].range + 1)
                {
                    conoutf(CON_WARN, "\f8faned\f7|detect: Client \f2%s \f5(%d)\f7, Type: \f2from.dist(to) > guns[gun].range + 1\f7, AKA: \f2Modified Gunrange", s->name, s->clientnum);
                }
                //end faned
                break;
            }
            ...
            case N_DELCUBE:
            {
                if(!d || iseditmuted(d->clientnum)) return;    //faned
                selinfo sel;
            ...
            case N_REMIP:
            {
                ...
                {
                    conoutf(CON_INFO, "\f8faned\f7|parsemessages: N_REMIP by client %s \fs\f5(%d)\fr was ignored", d->name, d->clientnum);
                    break;
                }
                else
                {
                    conoutf(CON_INFO, "\f8faned\f7|parsemessages: %s remipped", colorname(d));
                    mpremip(false);
                }
                break;
                //end faned
            ...
            case N_EDITMODE:
            {
                ...
                {
                    conoutf(CON_WARN,
                       "\f8faned\f7|detect: Client \f2%s \f5(%d)\f7, Type: \f2N_EDITMODE\f7, AKA: \f2Edittoggle in non edit game mode",
                       d->name, d->clientnum);
                }
                //end faned
                int val = getint(p);
            ...
            case N_NEWMAP:
            {
                ...
                {
                    conoutf(CON_INFO, "\f8faned\f7|parsemessages: N_NEWMAP by client %s \fs\f5(%d)\fr was ignored", d->name, d->clientnum);
                    break;
                }
                //end faned
            ...
            case N_INITAI:
            {
                ...
                if(receiveai)
                {
                    conoutf(CON_INFO, "\f8faned\f7|parsemessages: N_INITAI %s \fs\f5[%d]\fr (Skill: %d) by cn %d was ignored", name, bn, sk, on);
                    break;
                }
                //end faned
    ...
    void receivefile(packetbuf &p)
    {
        ...
        while(p.remaining()) switch(type = getint(p))
        {
            ...
            case N_SENDMAP:
            {
                if(receivesendmap)
                {
                        conoutf(CON_INFO, "\f8faned\f7|receivefile: N_SENDMAP was ignored");
                        return;
                }
                if(!m_edit) return;
                string oldname;
                ...
                if(!_getfile)
                {
                    ...
                    if(!map) return;
                    conoutf(CON_INFO, "\f8faned\f7|receivefile: received map");
                    ucharbuf b = p.subbuf(p.remaining());
                ...
                else
                {
                    ...
                    stream *map = openrawfile(path(fname), "wb");
                    if(!map) return;
                    conoutf(CON_INFO, "\f8faned\f7|receivefile: received file %s", fname);
                    ucharbuf b = p.subbuf(p.remaining());
                    map->write(b.buf, b.maxlen);
                    map->putchar('\0');
                    delete map;
    ...
    void getmap()
    {
        if(!m_edit) { conoutf(CON_ERROR, "\f8faned\f7|getmap: \"getmap\" only works in coop edit mode"); return; }
        conoutf("\f8faned\f7|getmap: getting map...");
        _getfile = false;
        addmsg(N_GETMAP, "r");
    }
    COMMAND(getmap, "");
    ...
    {
        if(!m_edit) { conoutf(CON_ERROR, "\f8faned\f7|getfile: \"getfile\" only works in coop edit mode"); return; }
        formatstring(filename)("%s", file);
        if(!file[0]) formatstring(filename)("getfile_%d", lastmillis);
        conoutf("\f8faned\f7|getfile: getting file...");
        _getfile = true;
        addmsg(N_GETMAP, "r");
    }
    ...
    void sendmap(bool full)
    {
        ...
        {
            conoutf(CON_ERROR, "\f8faned\f7|sendmap: \"sendmap\" only works in coop edit mode");
            return;
        }
        ...
        if(map)
        {
            ...
            if(len > 32*1024*1024) conoutf(CON_ERROR, "\f8faned\f7|sendmap: map is too large");
            else if(len <= 0) conoutf(CON_ERROR, "\f8faned\f7|sendfile: could not read map");
            else
            {
                conoutf("\f8faned\f7|sendmap: sending map (%4.3f KB) ...", (float(map->size())/1024.f));
                sendfile(-1, 2, map);
                if(needclipboard >= 0) needclipboard++;
            }
            delete map;
        }
        else conoutf(CON_ERROR, "\f8faned\f7|sendfile: could not read map");
        remove(findfile(fname, "rb"));
    }
    ...
    void sendfile(char *file)
    {
        if(!m_edit || (player1->state==CS_SPECTATOR && remote && !player1->privilege))
        {
            conoutf(CON_ERROR, "\f8faned\f7|sendfile: \"sendfile\" only works in coop edit mode");
            return;
        }
        defformatstring(fname)("%s", file);
        if(!file[0])
        {
            conoutf(CON_ERROR, "\f8faned\f7|sendfile: no file to send");
            return;
        }
        stream *map = openrawfile(path(fname), "rb");
        if(map)
        {
            stream::offset len = map->size();
            if(len > 32*1024*1024) conoutf(CON_ERROR, "\f8faned\f7|sendfile: file is too large");
            else if(len <= 0) conoutf(CON_ERROR, "\f8faned\f7|sendfile: could not read file");
            else
            {
                conoutf("\f8faned\f7|sendfile: sending file (%4.3f KB) ...", (float(map->size())/1024.f));
                sendfile(-1, 2, map);
                if(needclipboard >= 0) needclipboard++;
            }
            delete map;
        }
        else conoutf(CON_ERROR, "\f8faned\f7|sendfile: could not read file");
    }
    ICOMMAND(sendfile, "s", (char *file), sendfile(file));

    FANED_VARP(gotodist, INT_MIN, -32, INT_MAX);
    FANED_VARP(gotoguard, 0, 1, 1);

    void gotoplayer(const char *arg)
    {
        if(player1->state!=CS_SPECTATOR && player1->state!=CS_EDITING && gotoguard && gamemode)
        {
            conoutf(CON_ERROR, "\f8faned\f7|gotoplayer: \"gotoplayer\" only works in coop edit mode");
            return;
        }
    ...
    void gotosel()
    {
        if(player1->state!=CS_EDITING && gotoguard)
        {
            conoutf(CON_ERROR, "\f8faned\f7|gotosel: \"gotosel\" only works in coop edit mode");
            return;
        }
        player1->o = getselpos();
        vec dir;
