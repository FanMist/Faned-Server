...
//start faned
FANED_HVARP(bloodcolor, 0, 0x9F0000, 0xFFFFFF);
FANED_FVARP(bloodintensity, 0, 7.0f, INT_MAX);
FANED_HVARP(shotcolor, 0, 0x222222, 0xFFFFFF);
FANED_VARP(shotcolorrainbow, 0, 0, 1);
FANED_VARP(shotduration, 1, 500, 10000);
FANED_VARP(shotgravity, -50, 20, 50);
FANED_FVARP(shotwidth, 0, 0.6f, 50.0f);
FANED_VARP(shotsparks, 0, 1, 1);
FANED_VARP(shotstyle, 0, 1, 6);
//end faned
...
namespace game
{
    ...
    VARP(maxdebris, 10, 75, 1000);    //faned
    ...
    void bounced(physent *d, const vec &surface)
    {
        ...
        b->bounces++;
        int red = bloodcolor / 0x10000;    //faned
        int green = bloodcolor % 0x10000 / 0x100;    //faned
        int blue = bloodcolor % 0x100;    //faned
        adddecal(DECAL_BLOOD, vec(b->o).sub(vec(surface).mul(b->radius)), surface, 2.96f/b->bounces, bvec(~red, ~green, ~blue), rnd(4));    //faned
    }
    ...
    void updatebouncers(int time)
    {
        loopv(bouncers)
        {
            ...
            if(bnc.bouncetype==BNC_GRENADE && bnc.vel.magnitude() > 50.0f)
            {
                ...
                regular_particle_splash(PART_SMOKE, 1, 150, pos, shotcolorrainbow ? rnd(16777216) : shotcolor, 2.4f, 50, -20);    //faned
            }
            vec old(bnc.o);
            bool stopped = false;
    ...
    void damageeffect(int damage, fpsent *d, bool thirdperson)
    {
        ...
        if(blood) particle_splash(PART_BLOOD, damage/10, 1000, p, ~bloodcolor, bloodintensity);    //faned
        if(thirdperson)
        {
        ...
        loopv(projs)
        {
            ...
            if(!exploded)
            {
                ...
                else
                {
                    ...
                    else regular_particle_splash(PART_SMOKE, 2, 300, pos, shotcolorrainbow ? rnd(16777216) : shotcolor, 2.4f, 50, -20);    //faned
                }
            }
            if(exploded)
            {
        ...
        switch(gun)
        {
            ...
            case GUN_SG:
            {
                ...
                if(muzzleflash && d->muzzle.x >= 0)
                    particle_splash(PART_SMOKE, 3, 500, d->muzzle, shotcolorrainbow ? rnd(16777216) : shotcolor, 1.4f, 50, 501);
            ...
            case GUN_RL:
                if(muzzleflash && d->muzzle.x >= 0)
                {
                    ...
                    regular_particle_splash(PART_SMOKE, 1, 150, d->o, shotcolorrainbow ? rnd(16777216) : shotcolor, 2.4f, 50, -20);
                }
            ...
            case GUN_RIFLE:
                if(d!=hudplayer()) sound_nearmiss(S_NEARMISS1+rnd(3), from, to);
                if(shotsparks) particle_splash(PART_SPARK, 200, 250, to, 0xB49B4B, 0.24f);
                if(shotstyle == 1) particle_trail(PART_SMOKE, shotduration, hudgunorigin(gun, from, to, d), to, shotcolorrainbow ? rnd(16777216) : shotcolor, shotwidth, shotgravity); // smoke;
                if(shotstyle == 2) particle_trail(PART_STEAM, shotduration, hudgunorigin(gun, from, to, d), to, shotcolorrainbow ? rnd(16777216) : shotcolor, shotwidth, shotgravity); // steam;
                if(shotstyle == 3) particle_trail(PART_SPARK, shotduration, hudgunorigin(gun, from, to, d), to, shotcolorrainbow ? rnd(16777216) : shotcolor, shotwidth, shotgravity); // spark;
                if(shotstyle == 4) particle_trail(PART_SNOW, shotduration, hudgunorigin(gun, from, to, d), to, shotcolorrainbow ? rnd(16777216) : shotcolor, shotwidth, shotgravity); // snow;
                if(shotstyle == 5) particle_flare(hudgunorigin(gun, from, to, d), to, shotduration, PART_STREAK, shotcolorrainbow ? rnd(16777216) : shotcolor, shotwidth); // laser;
                if(shotstyle == 6) particle_flare(hudgunorigin(gun, from, to, d), to, shotduration, PART_LIGHTNING, shotcolorrainbow ? rnd(16777216) : shotcolor, shotwidth); // lightning;
                if(muzzleflash && d->muzzle.x >= 0 && shotstyle == 1)
                {
    ...
    void renderprojectiles()
    {
        ...
        loopv(projs)
        {
            ...
            particle_fireball(pos, 1.5f, PART_EXPLOSION, 15, 0xFF8080, 1.0f);    //faned
        }
    }
