...
namespace entities
{
    ...
    //start faned
    VARP(pickupautoswitch, 0, 0, 1);

    void switchgun(int type)
    {
        switch(type)
        {
            case I_BULLETS: { gunselect(GUN_CG, player1); break; }
            case I_CARTRIDGES: { gunselect(GUN_PISTOL, player1); break; }
            case I_GRENADES: { gunselect(GUN_GL, player1); break; }
            case I_ROCKETS: { gunselect(GUN_RL, player1); break; }
            case I_ROUNDS: { gunselect(GUN_RIFLE, player1); break; }
            case I_SHELLS: { gunselect(GUN_SG, player1); break; }
        }
    }
    //end faned
    ...
    void pickupeffects(int n, fpsent *d)
    {
        ...
        if(d==player1) switch(type)
        {
            ...
            case I_ROUNDS:
            case I_SHELLS:
                if(pickupautoswitch) switchgun(type);
                break;
            //end faned

            case I_BOOST:
                //conoutf(CON_GAMEINFO, "\f2you have a permanent +10 health bonus! (%d)", d->maxhealth);    //faned
                playsound(S_V_BOOST, NULL, NULL, 0, 0, 0, -1, 0, 3000);
                break;

            case I_QUAD:
                //conoutf(CON_GAMEINFO, "\f2you got the quad!");    //faned
